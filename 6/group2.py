...
groups = []
i = 0
while i < len(splitted_input):
    line = splitted_input[i]
    j = 1
    if len(line) > 0:
        sub_group = [line]

        if i < len(splitted_input):
            while len(splitted_input[i + j]) > 0:
                sub_group.append(splitted_input[i + j])
                j += 1
...
for group in groups:
    ...
    for subgroup in group:
        for c in subgroup:
            in_all = True
            for other_elements in group:
                in_all = in_all and (c in other_elements)
            if in_all:
                out += c
    test_list.append("".join(set(out)))
print(test_list)
summed_intersection = 0
for intersection in test_list:
    summed_intersection+=len(intersection)
print(summed_intersection)
