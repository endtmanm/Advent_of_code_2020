...
for line in lines:
    ...
    bag_content = [re.sub('bag\S*', "bags", content)[2:] for content in bag_content]
    # number_of_bags = [content for content in bag_content]
    # print(bag_name, "\t", bag_content)
    rules[bag_name] = bag_content
...
for bag, content in rules.items():
    for inner_bag in content:
        if inner_bag == "shiny gold bags":
            start_points.append(bag)

print(start_points)


def get_inner_bag(rules, inner_bag_name, score):
    for rule, bags in rules.items():
        if inner_bag_name in bags:
            score = get_inner_bag(rules, rule, score)
...
for start in start_points:
    score = get_inner_bag(rules, start, score)
