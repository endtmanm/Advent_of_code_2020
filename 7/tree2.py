...
input="""mirrored gold bags contain 3 light teal bags.
...
for line in lines:
    ...
    contain_length = len("contain ")
    bag_name = line[:contain_index - 1]
    bag_content = line[contain_index + contain_length:].split(", ")
    bag_content = [re.sub('bag\S*', "bags", content) for content in bag_content]
    cleaned_bag_content = []
    for content in bag_content:
        bag_number = 0
        content_name = ""
        try:
            bag_number = int(content[:2])
        except:
            pass
        content_name = content[2:]
        cleaned_bag_content.append((bag_number, content_name))
    rules[bag_name] = cleaned_bag_content

for bag, content in rules.items():
    print(bag, "\t", content)

start_points = rules["shiny gold bags"]


def get_amount_of_inner_bags(rules, inner_bag_name):
    bags = rules[inner_bag_name]
    score = 0
    for bag in bags:
        if bag[0] != 0:
            score += bag[0]
            multiplicator = get_amount_of_inner_bags(rules, bag[1])
            score += multiplicator * bag[0]
        else:
            return 0
    return score
...
for start in start_points:
    end_score += start[0]
    mutliplicator = get_amount_of_inner_bags(rules, start[1])
    end_score += mutliplicator * start[0]
print(end_score)
